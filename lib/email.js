...
<body style="margin:0;padding:0;background:#0a0a0a;font-family:'Segoe UI',sans-serif;color:#e0e0e0">
  <div style="max-width:560px;margin:0 auto;padding:40px 20px">
    ...
    <div style="background:#111;border-radius:12px;padding:20px;margin-bottom:20px">
      ...
      <p style="margin:0;color:#ccc;font-size:0.85rem;line-height:1.6">${shipping.direccion ? shipping.direccion + "<br>" : ""}${[shipping.comuna, shipping.region].filter(Boolean).join(", ")}</p>
      ${shipping.notas ? `<p style="margin:8px 0 0;color:#777;font-size:0.78rem">Nota: ${shipping.notas}</p>` : ""}
    </div>` : `
    <div style="background:#111;border-radius:12px;padding:20px;margin-bottom:20px">
...
export async function sendAbandonedCartEmail({ to, name, order, items, total }) {
  ...
  const bodyHtml = `
    <div style="background:#111;border-radius:12px;padding:28px;margin-bottom:20px">
      <h2 style="margin:0 0 8px;font-size:1.15rem;color:#fff">${name ? name + ", t" : "T"}u pedido te espera 👀</h2>
