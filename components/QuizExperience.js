...
export default function QuizExperience({ embed = false }) {
  const { perfumes } = useCatalog();
  // En el home (embed) el quiz parte directo en la primera pregunta.
  const [empezado, setEmpezado] = useState(embed);
  ...
  function finalizar(nuevas) {
    ...
    // Vercel Analytics solo acepta valores planos: "familia" (array) se serializa.
    track("quiz_complete", {
      ...nuevas,
      familia: Array.isArray(nuevas.familia) ? nuevas.familia.join("/") : nuevas.familia ?? null,
    });
    try {
  ...
  if (resultados) {
    ...
    return (
      <div className="quiz-wrap quiz-wrap-wide">
        <div className="quiz-head" style={{ textAlign: embed ? "center" : "left" }}>
          {!embed && <div className="kicker">Quiz de fragancias</div>}
          <h2 className="quiz-title">Tu selección personalizada</h2>
          <p className="quiz-sub" style={embed ? { marginLeft: "auto", marginRight: "auto" } : undefined}>
            Según tus respuestas, estas fragancias son para ti.
          </p>
          {chips.length > 0 && (
            <div className="quiz-chips" style={embed ? { justifyContent: "center" } : undefined}>
              {chips.map((c) => (
    ...
    return (
      <div className="quiz-wrap">
        <div className="quiz-head" style={{ textAlign: "center" }}>
          ...
          <h2 className="quiz-title">Preparando tus recomendaciones…</h2>
          <p className="quiz-sub" style={{ margin: "0 auto" }}>Estamos analizando tus respuestas.</p>
        </div>
      </div>
    );
  }

  // ---------- Bienvenida (solo página /quiz) ----------
  if (!empezado) {
    return (
      <div className="quiz-wrap">
        <div className="quiz-head" style={{ textAlign: "center" }}>
          ...
          <p className="quiz-sub" style={{ margin: "0 auto" }}>
    ...
    <div className="quiz-wrap">
      ...
      <div className={`quiz-card ${saliendo ? "leaving" : ""}`} key={pregunta.id}>
        <div className="quiz-head">
          ...
          <h3 className="quiz-question">{pregunta.pregunta}</h3>
