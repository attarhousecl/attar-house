...
function Backdrop({ seed }) {
  const { sceneId, angle, lightX, lightY, tint, shadowScale, dots, leaves } = seed;
    ...
    return (
      <div style={{ ...base, background: `linear-gradient(${angle}deg, #15120c, #050402 60%)` }}>
        ...
        <svg width="100%" height="100%" style={{ position: "absolute", inset: 0, opacity: 0.5 }}>
          ...
          <rect width="100%" height="100%" fill="url(#vein)" />
        </svg>
      </div>
    );
  }
    ...
    return (
      <div style={{ ...base, background: `linear-gradient(${angle}deg, #0c0a06, #1a140a)` }}>
        <svg width="100%" height="100%" viewBox={`0 0 ${SIZE} ${SIZE}`} style={{ position: "absolute", inset: 0 }}>
          ...
          {dots.map((d, i) => (
            <circle key={i} cx={(d.x / 100) * SIZE} cy={(d.y / 100) * SIZE} r={d.r} fill="url(#dot)" opacity={d.o} />
          ))}
        </svg>
      </div>
    );
  ...
  if (sceneId === "tropical") {
    return (
      <div style={{ ...base, background: `linear-gradient(${angle}deg, #0a1209, #060805)` }}>
        ...
        <svg width="100%" height="100%" viewBox={`0 0 ${SIZE} ${SIZE}`} style={{ position: "absolute", inset: 0 }}>
          {leaves.map((l, i) => (
            <g key={i} transform={`translate(${(l.x / 100) * SIZE},${(l.y / 100) * SIZE}) rotate(${l.rot}) scale(${l.scale})`} opacity={l.o}>
...
function GroundShadow({ shadowScale }) {
  const cx = SIZE / 2, cy = SIZE * 0.88;
  const rx = SIZE * 0.16 * shadowScale, ry = SIZE * 0.034 * shadowScale;
  return (
    <svg width={SIZE} height={SIZE} style={{ position: "absolute", inset: 0 }}>
      ...
      <ellipse cx={cx} cy={cy} rx={rx} ry={ry} fill="url(#gshadow)" />
    </svg>
  );
}
